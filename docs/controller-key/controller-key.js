...
function base64ToBytes(value) {
  if (typeof value !== "string" || value.length > 128 * 1024) throw new Error("The encrypted key contains invalid binary data.");
  let binary; try { binary = atob(value); } catch (error) { throw new Error("The encrypted key contains invalid binary data.", { cause: error }); }
...
async function generateControllerKey({ controllerId, passphrase, supersedesKeyId = null } = {}) {
  ...
  const ciphertext = await crypto.subtle.encrypt(
    { name: "AES-GCM", iv, additionalData: canonicalBytes(publicPackage), tagLength: 128 }, encryptionKey, pkcs8,
  );
  pkcs8.fill(0);
...
async function loadControllerKey(keyPackage, passphrase) {
  if (keyPackage?.format !== "mp-opt-controller-private-key-v1") throw new Error("Select a controller key generated by this browser tool.");
  validatePublicPackage(keyPackage.public_package);
  if (keyPackage.kdf?.name !== "PBKDF2" || keyPackage.kdf.hash !== "SHA-256" || keyPackage.kdf.iterations !== ITERATIONS) throw new Error("The encrypted key uses an unsupported derivation policy.");
  if (keyPackage.cipher?.name !== "AES-GCM" || keyPackage.cipher.tag_length !== 128) throw new Error("The encrypted key uses an unsupported cipher policy.");
  if (typeof passphrase !== "string" || passphrase.length < 16) throw new Error("Enter the key passphrase.");
  ...
  try {
    pkcs8 = new Uint8Array(await crypto.subtle.decrypt(
      ...
      encryptionKey, base64ToBytes(keyPackage.ciphertext),
    ));
  ...
  const fingerprint = await sha256Hex(textEncoder.encode(publicKey));
  if (publicKey !== keyPackage.public_package.public_key || fingerprint !== keyPackage.public_package.public_key_sha256 || `ek-${fingerprint.slice(0, 16)}` !== keyPackage.public_package.key_id) throw new Error("The encrypted private key does not match its public package.");
...
if (typeof document !== "undefined") {
  ...
  generateButton.addEventListener("click", async () => {
    ...
    const repeated = document.getElementById("generate-passphrase-repeat").value;
    ...
    try {
      ...
      generated = await generateControllerKey({
        controllerId: document.getElementById("controller-id").value.trim(), passphrase,
        supersedesKeyId: document.getElementById("supersedes-key").value.trim() || null,
      });
      document.getElementById("generate-passphrase").value = ""; document.getElementById("generate-passphrase-repeat").value = "";
      document.getElementById("generation-title").textContent = "Key generated and recovery-tested";
      document.getElementById("generation-summary").textContent = "Download both files now. Retain the encrypted private package under controller custody; share only the public package.";
  ...
  document.getElementById("private-download").addEventListener("click", () => generated && downloadJson(`${generated.publicPackage.key_id}.controller-key.json`, generated.privatePackage));
  ...
  signButton.addEventListener("click", async () => {
    ...
    try {
      const keyPackage = await readJsonFile(document.getElementById("sign-private-file"), "encrypted controller-key package");
      const signingDocument = await readJsonFile(document.getElementById("sign-document-file"), "Server document");
      signed = await signControllerDocument(keyPackage, document.getElementById("sign-passphrase").value, signingDocument);
